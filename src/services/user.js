   ...
   */
    async withdrawFunds(user, withdrawalAmount) {
        ...
        try {
              ...
              updateWallet = await user.updateOne({'wallet.balance':newBalance},{returnOriginal:false}, {session})
    ...
    async transferFunds(user, recipient, amount) {
        ...
        try {
            ...
            if(!sourceWalletCollection ){
              ...
              const sourceNewBalance = (+sourceWalletCollection.balance.toString()) - transferAmount
              const destinationNewBalance = (+destinationWalletCollection.balance.toString()) + transferAmount
              const updateSourceWallet = await user.updateOne({'wallet.balance':sourceNewBalance}, {session})
              const updateDestinationWallet = await recipientWallet.updateOne({'wallet.balance':destinationNewBalance}, {session})
